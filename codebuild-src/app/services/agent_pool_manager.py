...
from app.models import AgentPool, AgentNumber, CallLog, DIDPool
...
class AgentPoolManager:
    ...
    def _validate_personality_config(self, config: Dict[str, Any]) -> None:
        ...
        for field in required_fields:
            if field not in config:
                raise ValueError(
                    f"Missing required personality field: {field}")
        ...
        if config['conversation_style'] not in valid_styles:
            raise ValueError(
                f"Invalid conversation style: {config['conversation_style']}")
    ...
    async def get_optimal_agent_for_call(
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                logger.info(
                    f"Selected agent {selected_agent.name} for call to {target_phone}")
    ...
    async def _has_healthy_numbers(
            self,
            session: AsyncSession,
            agent_id: UUID) -> bool:
    ...
    async def _calculate_geographic_score(
        ...
        regional_score = self._calculate_regional_proximity(
            agent.region, area_code)
        return regional_score

    def _calculate_regional_proximity(
            self,
            agent_region: str,
            area_code: str) -> float:
    ...
    def _weighted_agent_selection(
            self, scored_agents: List[tuple]) -> AgentPool:
        """Select agent using weighted random selection"""
        if not scored_agents:
            return None
    ...
    async def _update_agent_state_for_call(self, agent_id: UUID) -> None:
        ...
        if agent_state['daily_call_count'] >= max_calls_per_hour:
            ...
            agent_state['rest_until'] = datetime.utcnow() + \
                timedelta(hours=rest_hours)
    ...
    async def complete_call(
        ...
        if agent_id in self.active_agents:
            ...
            self.active_agents[agent_id]['current_calls'] = max(
                0, self.active_agents[agent_id]['current_calls'])
    ...
    async def _update_agent_performance(
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                logger.info(
                    f"Updated performance for agent {agent_id}: success={new_success_rate:.2f}, answer={new_answer_rate:.2f}")
    ...
    async def get_agent_performance_summary(
            self, agent_id: UUID) -> Dict[str, Any]:
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                call_stats_query = select(
                    func.count(
                        CallLog.id).label('total_calls'), func.sum(
                        func.case(
                            (CallLog.call_status == 'completed', 1), else_=0)).label('successful_calls'), func.sum(
                        func.case(
                            (CallLog.call_answered.is_(True), 1), else_=0)).label('answered_calls'), func.avg(
                ...
                return {
                    ...
                    'current_state': self.active_agents.get(
                        agent_id,
                        {})}
