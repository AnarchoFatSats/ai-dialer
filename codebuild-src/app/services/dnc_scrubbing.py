...
from app.config import DNC_REGISTRY_CONFIG
import re
...
class DNCScrubbingService:
        ...
        try:
            ...
            for source_name, processor in self.dnc_sources.items():
                try:
                    ...
                    logger.info(
                        f"Processed {count} numbers from {source_name}")
    ...
    async def _process_state_dnc(self, source_name: str) -> int:
        ...
        try:
            ...
            state_config = DNC_REGISTRY_CONFIG['state_registries'].get(
                state_code)
            if not state_config:
    ...
    async def _download_state_dnc(
            self,
            state_code: str,
            registry_url: str) -> List[str]:
        """
    ...
    async def _download_texas_dnc(
            self, client: httpx.AsyncClient) -> List[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Texas DNC download failed: {response.status_code}")
                return []
    ...
    async def _download_florida_dnc(
            self, client: httpx.AsyncClient) -> List[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Florida DNC download failed: {response.status_code}")
                return []
    ...
    async def _download_newyork_dnc(
            self, client: httpx.AsyncClient) -> List[str]:
    ...
    async def _download_california_dnc(
            self, client: httpx.AsyncClient) -> List[str]:
    ...
    async def _add_or_update_dnc_entry(
            self,
            phone_number: str,
            registry_source: str,
        ...
        try:
            ...
            logger.error(
                f"Error adding/updating DNC entry {phone_number}: {e}")
    ...
    async def _cleanup_old_dnc_entries(self) -> int:
        ...
        try:
            ...
            ) - timedelta(days=DNC_REGISTRY_CONFIG['max_age_days'])

            stmt = select(DNCRegistry).where(
                DNCRegistry.last_updated < cutoff_date)
            result = await self.session.execute(stmt)
    ...
    async def check_phone_dnc_status(
            self, phone_number: str) -> Tuple[bool, Optional[str]]:
        """
    ...
    async def add_company_suppression_numbers(
            self, phone_numbers: List[str]) -> int:
        """
        ...
        try:
            ...
            logger.info(
                f"Added {added_count} numbers to company suppression list")
            return added_count
