# AI Dialer - AWS CodeBuild Deployment (Option B) - Fixed
...
if ($service_exists -eq $SERVICE_NAME) {
    ...
    $SUBNET_LIST = ($SUBNET_IDS -split "\s+" | ForEach-Object { "`"$_`"" }) -join ","
    ...
    $networkConfig = "awsvpcConfiguration={subnets=[$SUBNET_LIST],securityGroups=[`"$SG_ID`"],assignPublicIp=ENABLED}"
    
    aws ecs create-service --cluster $CLUSTER_NAME --service-name $SERVICE_NAME --task-definition "ai-dialer-task" --desired-count 1 --launch-type FARGATE --network-configuration $networkConfig --region $AWS_REGION
}
...
Write-Host "2. Test AI functionality: python test-production-ai-ecs.py" -ForegroundColor White
Write-Host "3. Frontend team can now connect!" -ForegroundColor White
