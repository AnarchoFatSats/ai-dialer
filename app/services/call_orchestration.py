...
class CallOrchestrationService:
    ...
    async def start_orchestration(self):
        ...
        self.orchestration_task = asyncio.create_task(
        ...
        try:
            ...
            async with get_db() as db:
                ...
                if not campaign or campaign.status != 'active':
                    logger.warning(
                        f"Campaign {campaign_id} not found or not active")
                    return False
                ...
                async with get_dnc_scrubbing_service() as dnc_service:
                    ...
                    if is_dnc:
                        logger.info(
                            f"Lead {lead_id} is on DNC list ({source}), skipping")
                        return False
                ...
                logger.info(
                    f"Queued call for lead {lead_id} in campaign {campaign_id}")
                return True
            ...
            while (current_calls < self.max_concurrent_calls and
                ...
                if success:
                    ...
                    if call_request.retry_count < call_request.max_retries:
                        ...
                        self.call_queue.append(call_request)
                        logger.info(
                            f"Retrying call for lead {call_request.lead_id} in 5 minutes")
    ...
    async def _initiate_call(self, call_request: CallRequest) -> bool:
        ...
        try:
            ...
            if not did:
                logger.warning(
                    f"No available DID for campaign {call_request.campaign_id}")
                return False
            ...
            logger.info(
                f"Initiated call {call_result['call_log_id']} for lead {call_request.lead_id}")
            return True
        ...
        try:
            ...
            async with get_cost_optimization_engine() as cost_engine:
                if not await cost_engine.check_budget_available(call_request.campaign_id):
                    logger.warning(
                        f"Budget exceeded for campaign {call_request.campaign_id}")
                    return False

            # Check lead status
            async with get_db() as db:
                lead_query = select(Lead).where(
                    Lead.id == call_request.lead_id)
                lead = await db.execute(lead_query)
                lead = lead.scalar_one_or_none()
            ...
            async with get_db() as db:
                campaign_query = select(Campaign).where(
                    Campaign.id == call_request.campaign_id)
                campaign = await db.execute(campaign_query)
                campaign = campaign.scalar_one_or_none()

                if not campaign or campaign.status != 'active':
                    logger.warning(
                        f"Campaign {call_request.campaign_id} not active")
                    return False

            # Check calling hours
            if not self._is_calling_hours_valid(lead):
                logger.info(
                    f"Outside calling hours for lead {call_request.lead_id}")
                return False
        ...
        try:
            ...
            for call_log_id, call_data in self.active_calls.items():
                try:
                    ...
                    call_duration = (
                        datetime.utcnow() -
                        call_data['started']).total_seconds()

                    if call_duration > 300:  # 5 minute timeout
                        logger.warning(
                            f"Call {call_log_id} timed out after {call_duration} seconds")
    ...
    async def _handle_status_change(
            self, call_log_id: int, call_data: Dict[str, Any], new_status: CallStatus):
        """Handle call status changes"""
        try:
            logger.info(
                f"Call {call_log_id} status changed to {new_status.value}")
    ...
    async def _handle_call_timeout(
            self, call_log_id: int, call_data: Dict[str, Any]):
        """Handle call timeout"""
        try:
    ...
    async def handle_ai_disconnect(self, call_log_id: int):
        ...
        try:
            ...
            if call_log_id in self.active_calls:
                ...
                # self.current_capacity += 1 # This line was removed as per the
                # new_code, as self.current_capacity is not defined.

                # Adjusted to reflect actual active calls
                logger.info(
                    f"AI disconnected from call {call_log_id}, capacity freed up. Current capacity: {self.max_concurrent_calls - len(self.active_calls)}")
    ...
    async def _pause_campaign(self, campaign_id: int):
        ...
        try:
            async with get_db() as db:
                ...
                campaign_update = update(Campaign).where(
                    Campaign.id == campaign_id).values(
                    status='paused',
                ...
                logger.warning(
                    f"Paused campaign {campaign_id} due to budget limits")
    ...
    def _is_calling_hours_valid(self, lead: Any) -> bool:
        ...
        try:
            ...
            # This is a simplified check - in production, implement proper
            # timezone handling
        ...
        try:
            async with get_db() as db:
                ...
                total_transfers = await db.execute(
                    select(func.count(CallLog.id)).where(CallLog.transfer_attempted)
                )
                ...
                failed_transfers = await db.execute(
                    select(func.count(CallLog.id)).where(CallLog.transfer_failed)
                )
                ...
                transfer_success_rate = (
                    ...
                    total_transfers *
                    100) if total_transfers > 0 else 0
