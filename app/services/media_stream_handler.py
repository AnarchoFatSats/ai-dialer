...
class MediaStreamHandler:
    ...
    async def handle_media_stream(self, websocket, path: str):
        ...
        try:
            ...
            logger.info(f"Media stream connection closed for call {call_log_id}")
    ...
    async def _process_media_message(self, call_log_id: int, message: str):
        ...
        try:
            ...
            if event == 'connected':
                ...
                self.active_streams[call_log_id]['stream_sid'] = data.get('streamSid')
                self.active_streams[call_log_id]['call_sid'] = data.get('start', {}).get('callSid')
    ...
    async def _process_audio_data(self, call_log_id: int, media_data: Dict[str, Any]):
        ...
        try:
            ...
            # Process when we have enough audio data (adjust based on sample rate)
    ...
    async def _send_initial_greeting(self, call_log_id: int, conversation_context):
