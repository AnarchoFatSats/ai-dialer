...
class AWSConnectMediaHandler:
    ...
    async def handle_connect_media_stream(self, websocket, path: str):
        ...
        try:
            ...
            logger.info(f"New Amazon Connect media stream connection for call {call_log_id}")
            ...
            logger.info(f"Amazon Connect media stream connection closed for call {call_log_id}")
    ...
    async def _process_connect_media_message(self, call_log_id: int, message: str):
    ...
    async def _handle_stream_start(self, call_log_id: int, data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"Stream started for call {call_log_id}, contact {stream_info['contact_id']}")
    ...
    async def _handle_stream_stop(self, call_log_id: int, data: Dict[str, Any]):
    ...
    async def _handle_stream_connected(self, call_log_id: int, data: Dict[str, Any]):
    ...
    async def _send_initial_greeting(self, call_log_id: int, conversation_context: Dict[str, Any]):
        """Send initial AI greeting through Amazon Connect"""
        try:
            # Generate greeting text
            greeting_text = conversation_context.get('greeting', 'Hello, thank you for your time.')
    ...
    async def _send_audio_to_connect(self, call_log_id: int, audio_bytes: bytes):
        ...
        try:
            ...
            async with get_db() as db:
                ...
                if not campaign or not campaign.transfer_number:
                    logger.warning(f"No transfer number configured for campaign {call_log.campaign_id}")
                    return
    ...
    async def _setup_call_recording(self, call_log_id: int, stream_arn: str):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Created Kinesis Video Stream for call {call_log_id}: {stream_name}")
                ...
                if e.response['Error']['Code'] == 'ResourceInUseException':
                    logger.info(f"Kinesis Video Stream already exists: {stream_name}")
                else:
