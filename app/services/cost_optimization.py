...
from app.models import (
    Campaign, CallLog, CostOptimization, CallStatus, CallDisposition,
    CampaignStatus
)
...
class CostOptimizationEngine:
    ...
    async def track_realtime_costs(
            self, campaign_id: uuid.UUID) -> CostMetrics:
        """
        ...
        try:
            ...
            logger.info(
                f"Cost tracking for campaign {campaign_id}: ${cost_metrics.total_cost:.4f} total, {cost_metrics.budget_utilization:.1f}% budget used")
            return cost_metrics

        except Exception as e:
            logger.error(
                f"Error tracking costs for campaign {campaign_id}: {e}")
            raise

    async def _get_campaign(
            self,
            campaign_id: uuid.UUID) -> Optional[Campaign]:
        """Get campaign by ID."""
    ...
    async def _calculate_cost_metrics(
        ...
        budget_utilization = (
            total_cost /
            campaign.max_daily_budget *
            100) if campaign and campaign.max_daily_budget > 0 else 0
    ...
    async def _check_cost_alerts(
            self, campaign_id: uuid.UUID, metrics: CostMetrics) -> List[Dict[str, Any]]:
        """
    ...
    def _should_auto_pause(self, metrics: CostMetrics,
    ...
    async def _auto_pause_campaign(
            self, campaign_id: uuid.UUID, alerts: List[Dict[str, Any]]):
        """
        ...
        try:
            ...
            if campaign and campaign.status == CampaignStatus.ACTIVE:
                ...
                alert_messages = [alert['message'] for alert in alerts if alert.get(
                    'auto_action') == 'pause_campaign']
    ...
    async def _update_cost_optimization_record(
            self, campaign_id: uuid.UUID, metrics: CostMetrics, alerts: List[Dict[str, Any]]):
        """
        ...
        try:
            ...
            if existing_record:
                ...
                existing_record.recommended_actions = [
                    alert['recommendation'] for alert in alerts]
                existing_record.auto_pause_triggered = any(
                    alert.get('auto_action') == 'pause_campaign' for alert in alerts)
            else:
                # Create new record
    ...
    async def _get_cost_trends(
        ...
        daily_costs_stmt = select(
            func.date(
                CallLog.initiated_at).label('date'),
            func.sum(
                CallLog.total_cost).label('total_cost'),
            func.count(
                CallLog.id).label('total_calls'),
            func.count(
                    CallLog.id).filter(
                        CallLog.disposition == CallDisposition.TRANSFER).label('transfers'),
            func.avg(
                            CallLog.cost_per_minute).label('avg_cost_per_minute')).where(
                                CallLog.campaign_id == campaign_id,
    ...
    async def _get_efficiency_analysis(
            self, campaign_id: uuid.UUID, start_date: datetime) -> Dict[str, Any]:
        """Get efficiency analysis."""
        # Calculate efficiency metrics
        efficiency_stmt = select(
            func.sum(
                CallLog.total_cost).label('total_cost'), func.count(
                CallLog.id).label('total_calls'), func.count(
                CallLog.id).filter(
                    CallLog.status == CallStatus.ANSWERED).label('answered_calls'), func.count(
                        CallLog.id).filter(
                            CallLog.disposition == CallDisposition.TRANSFER).label('transfers'), func.sum(
                                CallLog.talk_time_seconds).label('total_talk_time')).where(
        ...
        if row and row.total_calls > 0:
            ...
            transfer_rate = (row.transfers / row.answered_calls) * \
                100 if row.answered_calls > 0 else 0
            cost_per_call = row.total_cost / row.total_calls
            cost_per_transfer = row.total_cost / row.transfers if row.transfers > 0 else 0
            cost_per_minute = row.total_cost / \
                (row.total_talk_time / 60) if row.total_talk_time > 0 else 0
    ...
    async def _get_budget_analysis(
        ...
        for row in daily_budget_result:
            utilization = (
                row.daily_cost /
                campaign.max_daily_budget *
                100) if campaign.max_daily_budget > 0 else 0
    ...
    async def _get_optimization_opportunities(
        ...
        if efficiency['cost_per_transfer'] > 0.14:
            savings_potential = (
                efficiency['cost_per_transfer'] - 0.14) * efficiency.get('transfers', 0)
    ...
    async def _get_cost_predictions(
        ...
        recent_costs = [day['total_cost']
                        for day in recent_data['daily_trends'] if day['total_cost'] > 0]
        if recent_costs:
            ...
            return {
                ...
                'confidence': min(len(recent_costs) * 25, 100)
            }
        else:
            return {
                'daily_cost_prediction': 0,
