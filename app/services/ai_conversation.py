...
class AIConversationEngine:
    ...
    async def process_audio_chunk(self, call_log_id: int, audio_data: bytes) -> Optional[bytes]:
        """Process incoming audio chunk and generate response"""
        try:
            if call_log_id not in self.active_conversations:
                logger.warning(f"No active conversation for call {call_log_id}")
                return None
    ...
    async def _generate_ai_response(self, context: ConversationContext, user_input: str) -> Optional[str]:
        """Generate AI response using Claude"""
        try:
            # Get campaign and lead data for context
            async with get_db() as db:
                campaign_query = select(Campaign).where(Campaign.id == context.campaign_id)
                campaign = await db.execute(campaign_query)
                campaign = campaign.scalar_one_or_none()
            ...
            conversation_context = self._build_conversation_context(context, campaign, lead)
    ...
    async def _generate_contextual_response(self, context: ConversationContext, 
        ...
        try:
            ...
            if context.state == ConversationState.GREETING:
                ...
                prompt = self._build_qualification_prompt(context, user_input, conversation_context)
            elif context.state == ConversationState.PRESENTATION:
                prompt = self._build_presentation_prompt(context, user_input, conversation_context)
            elif context.state == ConversationState.OBJECTION_HANDLING:
                prompt = self._build_objection_handling_prompt(context, user_input, conversation_context)
            elif context.state == ConversationState.CLOSING:
                prompt = self._build_closing_prompt(context, user_input, conversation_context)
            else:
                prompt = self._build_default_prompt(context, user_input, conversation_context)
    ...
    def _build_greeting_prompt(self, context: ConversationContext, user_input: str, conversation_context: str) -> str:
...
Generate your response:"""
    ...
    def _build_qualification_prompt(self, context: ConversationContext, user_input: str, conversation_context: str) -> str:
...
Generate your response:"""
    ...
    def _build_presentation_prompt(self, context: ConversationContext, user_input: str, conversation_context: str) -> str:
...
Generate your response:"""
    ...
    def _build_objection_handling_prompt(self, context: ConversationContext, user_input: str, conversation_context: str) -> str:
...
Generate your response:"""
    ...
    def _build_closing_prompt(self, context: ConversationContext, user_input: str, conversation_context: str) -> str:
...
Generate your response:"""
    ...
    def _build_default_prompt(self, context: ConversationContext, user_input: str, conversation_context: str) -> str:
...
Generate your response:"""
    ...
    def _build_conversation_context(self, context: ConversationContext, campaign: Any, lead: Any) -> str:
...
"""
    ...
    async def _analyze_conversation_state(self, context: ConversationContext, user_input: str):
        """Analyze conversation state and sentiment"""
        try:
            # Simple sentiment analysis based on keywords
            positive_words = ['yes', 'interested', 'good', 'great', 'sure', 'okay', 'sounds', 'tell']
            negative_words = ['no', 'not', 'busy', 'interested', 'time', 'remove', 'stop', 'don\'t']
            ...
            if positive_score > negative_score:
                context.sentiment_score = min(1.0, context.sentiment_score + 0.1)
            elif negative_score > positive_score:
                context.sentiment_score = max(-1.0, context.sentiment_score - 0.1)
            
            # Check for objections
            objection_keywords = ['no', 'not interested', 'busy', 'not now', 'remove', 'stop calling']
            ...
            transfer_keywords = ['speak to', 'talk to', 'transfer', 'manager', 'supervisor', 'human']
            ...
            if context.state == ConversationState.GREETING and len(context.conversation_history) > 2:
    ...
    def _update_conversation_state(self, context: ConversationContext, user_input: str, ai_response: str):
        """Update conversation state based on response"""
        try:
    ...
    async def _generate_initial_greeting(self, campaign: Any, lead: Any) -> str:
    ...
    async def end_conversation(self, call_log_id: int) -> Dict[str, Any]:
        ...
        try:
            if call_log_id in self.active_conversations:
                ...
                async with get_db() as db:
                    update_query = update(CallLog).where(CallLog.id == call_log_id).values(
                        qualified=context.qualified,
                        transfer_requested=context.transfer_requested,
                        objections_count=context.objections_count,
    ...
    def get_conversation_context(self, call_log_id: int) -> Optional[ConversationContext]:
