...
class AnalyticsEngine:
    ...
    async def _get_hourly_metrics(self) -> Dict[str, Any]:
        ...
        answer_rate = (
            answered_last_hour /
            calls_last_hour *
            100) if calls_last_hour > 0 else 0
        transfer_rate = (
            transfers_last_hour /
            answered_last_hour *
            100) if answered_last_hour > 0 else 0
    ...
    async def get_campaign_analytics(
            self, campaign_id: uuid.UUID, days: int = 7) -> Dict[str, Any]:
        """
    ...
    async def _get_campaign_basic_metrics(
        ...
        return {
            ...
            'cost_per_call': round(
                ...
                6) if total_calls > 0 else 0,
            'cost_per_transfer': round(
                total_cost / transfers,
    ...
    async def _get_campaign_time_analysis(
        ...
        hourly_stmt = select(
            func.extract(
                'hour',
                CallLog.initiated_at).label('hour'),
            func.count(
                CallLog.id).label('calls'),
            func.count(
                CallLog.id).filter(
                    CallLog.status == CallStatus.ANSWERED).label('answered'),
            func.count(
                        CallLog.id).filter(
                            CallLog.disposition == CallDisposition.TRANSFER).label('transfers')).where(
                                CallLog.campaign_id == campaign_id,
        ...
        daily_stmt = select(
            ...
            func.sum(
                                CallLog.total_cost).label('cost')).where(
                                    CallLog.campaign_id == campaign_id,
    ...
    async def _get_campaign_geo_analysis(
        ...
        area_code_stmt = select(
            ...
            func.count(
                    CallLog.id).filter(
                        CallLog.disposition == CallDisposition.TRANSFER).label('transfers')).select_from(
                            CallLog.join(Lead)).where(
                                CallLog.campaign_id == campaign_id,
                                CallLog.initiated_at >= start_date).group_by(
                                    Lead.area_code).order_by(
                                        func.count(
                                            CallLog.id).desc())
    ...
    async def _get_campaign_conversion_funnel(
        ...
        answer_rate = (
            answered_calls /
            contacted_leads *
            100) if contacted_leads > 0 else 0
        qualification_rate = (
            qualified_leads /
            answered_calls *
    ...
    async def record_realtime_metric(self,
                                     metric_name: str,
                                     metric_value: float,
